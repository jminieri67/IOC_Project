...
namespace IOC
{
    ...
    public class Container : IContainer
    {
        ...
        public void AddTransient<From, To>()
        {
            ...
            {
                throw new Exception($"AddTransient: Mapping Exception when registering {typeof(From).Name} for {typeof(To).Name}", ex);
            }
        }
        ...
        public void AddSingleton<From, To>()
        {
            ...
            {
                throw new Exception($"AddSingleton: Mapping Exception when registering {typeof(From).Name} for {typeof(To).Name}", ex);
            }
        }
        ...
        public object Create(Type type)
        {
            ...
            {
				throw new Exception($"Create: Object instance for {nameof(type)} could not be created", ex);
            }
		...
		private LifecycleType GetLifecycleType(Type type)
		{
			...
			{
				throw new Exception($"GetLifecycleType: {nameof(type)} could not be resolved", ex);
			}
		...
		private object GetTransient(Type type)
        {
            ...
            {
                throw new Exception($"GetTransient: {nameof(type)} could not be resolved", ex);
            }
        ...
        private object GetSingleton(Type type)
        {
            ...
            {
                throw new Exception($"GetSingleton: {nameof(type)} could not be resolved", ex);
            }
